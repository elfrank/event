...
class UserController extends AuthakeAppController {
  var $name     = 'User';
  var $uses     = array('Authake.User', 'Authake.Rule', 'Authake.Profile');
    ...
    function register() {
        if (!empty($this->data)) {
            ...
            if ($this->User->save($this->data)) {
				...
				$this->Profile->save(array('user_id' => $this->User->id));
...
//        $this->set('infos', $this->params['named']);
      ...
      if (!empty($this->data)) 
	{
	  if($this->User->save($this->data)) 
	    {
	      $this->Session->setFlash(__('Your account data has been saved', true));
	      $this->redirect(array('admin' => 0, 'plugin' => 'authake', 'controller' => 'profile', 'action' => 'index'));
	    } 
	  else 
	    {
